...
export default function CourseScreen() {
  ...
  const topTocard = sortedParticipants.find((c) =>
  ...
  const pronosticIa = sortedParticipants.slice(0, 5).map((c) => c.numero).join(" - ");
  ...
  const lectureCourse = () => {
    const scores = sortedParticipants.map((c) => c.scoreIA || 0);
  ...
  const strategieConseil = () => {
    ...
    const chances = sortedParticipants.slice(1, 4).map((c) => c.numero).join(" ");
    const outsiders = sortedParticipants
      .filter((c) => (c.badges || []).includes("TOCARD IA") || (c.value || 0) > 3)
      .slice(0, 3)
      .map((c) => c.numero)
    ...
    if (sortedParticipants.filter((c) => (c.value || 0) > 3).length >= 3) {
